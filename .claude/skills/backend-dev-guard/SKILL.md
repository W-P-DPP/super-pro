...
- 新增后端接口默认先判断是否属于通用平台能力，而不是先判断要放进哪个已有服务。
- 如果接口不专属于某个单独业务域，例如设备资源、运行环境信息、平台级公共能力、跨前端共用的基础查询或工具型接口，默认放入 `general-server`。
- 只有当接口和某个独立业务服务强耦合，离开该服务的领域模型、数据源、权限语义或业务流程就不成立时，才放入该专属服务，例如 `agent-server`、`reimburse-server`。
- 判断归属时优先看“领域边界”和“未来复用范围”，不要仅因为当前页面属于某个前端包，就把接口放进对应的专属后端。
- 当既有接口归属和上述原则冲突时，后续重构应优先向 `general-server` 收敛，并同步更新前后端调用路径与文档约束。
