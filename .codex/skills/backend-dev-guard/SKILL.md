...
- 后端新增接口如无特别说明，默认必须启用 JWT 鉴权；匿名接口必须在需求、设计或实现说明中显式标注
- 默认在 `general-server/src/index.ts` 挂载业务 router 时接入 `jwtMiddleware`；如果同一业务域同时存在匿名接口和受保护接口，必须在该业务 router 内显式区分，不允许回退到全局白名单
- 禁止通过在 `jwtMiddleware` 内维护路径白名单的方式放行匿名接口
...
后端新增接口默认按“受 JWT 保护”处理，默认在 `general-server/src/index.ts` 挂载业务 router 时接入 `jwtMiddleware`。
如果某个业务域包含匿名接口，必须在该业务 router 内显式把匿名接口放在 JWT 中间件之前或逐路由省略 JWT，并在变更上下文中说明匿名原因。
